...
const { getCompany, getCompanyUsers, createCompanyUser, updateCompany, updateCompanyStatus, deleteCompany } =
  usePlatformAdminApi();
  ...
  <div class="space-y-8">
    ...
    <DataPanel title="Основная информация" description="Карточка выбранной компании с настройками и статусом.">
      <form v-if="!loading && company" class="grid gap-4 md:grid-cols-2" @submit.prevent="saveCompany">
        ...
        <div class="grid gap-4 md:col-span-2 lg:grid-cols-4">
    ...
    >
      <form class="grid gap-4 md:grid-cols-2" @submit.prevent="submitUser">
        ...
        <label class="space-y-2">
          ...
          <USelect v-model="userForm.role" :items="COMPANY_ROLE_OPTIONS" value-key="value" :ui="softSelectUi" />
        </label>
        <label class="space-y-2">
