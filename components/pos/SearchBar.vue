<template>
  <div class="relative flex justify-between">
    <!-- Поиск -->
    <div
      class="relative z-30 flex items-center gap-4 p-[15px] w-[700px] bg-[#404040] rounded-[15px]"
    >
    ...
    <div class="relative z-30 flex gap-2">
      <button
        class="flex items-center justify-center bg-[#404040] p-[15px] rounded-[15px] hover:bg-[#5e5e5e] transition"
      >
        🔄
      </button>
      <button
    ...
    <div
      v-if="filteredProducts.length > 0 && searchQuery"
      class="absolute top-full mt-2 w-full rounded-xl shadow-lg max-h-[250px] overflow-y-auto z-20 flex flex-col gap-3"
    >
      <div
        v-for="product in filteredProducts"
        :key="product.id"
        class="p-3 cursor-pointer bg-[#262626] flex justify-between items-center text-[16px] font-semibold"
