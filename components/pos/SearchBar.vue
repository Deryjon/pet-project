  ...
  >
    ...
    >
      <div class="flex min-h-full flex-col">
        ...
        <div
          class="grid min-h-0 flex-1 grid-cols-1 gap-0 lg:grid-cols-[420px_minmax(0,1fr)]"
        >
              ...
              >
                <div class="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div class="mt-1 text-xs text-[#9a9a9a]">
                      {{ sale.clientName }} • {{ sale.sellerName }}
                    </div>
                    <div
                      class="mt-2 inline-flex rounded-full bg-white/5 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#bdbdbd]"
                    >
                      {{ sale.statusLabel }}
                    </div>
                  </div>
          ...
          <div class="min-h-0 overflow-y-auto p-5 sm:p-6">
            <div v-if="selectedSale" class="flex h-full flex-col">
              <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
                <div
                  class="rounded-[22px] border border-white/8 bg-[#262626] p-5"
                >
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      <div class="text-[22px] font-semibold">
                        Продажа #{{ selectedSale.number }}
                      </div>
                      <div class="mt-1 text-sm text-[#8f8f8f]">
                        Общая сумма: {{ formatPrice(selectedSale.total) }} UZS
                      </div>
                    </div>
                    <UButton
                  ...
                  <div class="mt-5 space-y-3">
                    <div
                      class="text-sm font-semibold uppercase tracking-[0.1em] text-[#8f8f8f]"
                    >
                      Корзина
                    </div>
                    <div
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <label
                            class="inline-flex items-center gap-2 text-sm text-[#d3d3d3]"
                          >
                            <input
                ...
                <div class="space-y-4">
                  ...
                  >
                    ...
                    <div class="mt-3 text-[16px] font-semibold text-white">
...
const availableSales = computed(() =>
  rawSales.value.filter(matchesCurrentMode),
);
  ...
  return selectedSale.value.items
    ...
    .map((item) => ({
      ...
      selectedQuantity: Math.min(
        item.quantity,
        Math.max(1, getSelectedQuantity(item.productId)),
      ),
    }));
});
...
function normalizeSale(raw: any): SaleListItem {
  ...
  return {
    ...
    paymentMethodId: String(
      raw?.payment_method ?? raw?.payment?.id ?? raw?.payment_type ?? "",
    ),
    paymentMethodName: String(
      raw?.payment?.name ?? raw?.payment_type_name ?? "Не указано",
    ),
    sellerId: String(
      raw?.seller_id ?? raw?.seller?.id ?? raw?.user_id ?? raw?.user?.id ?? "",
    ),
    createdAt,
    dateTimeLabel: `${new Date(createdAt).toLocaleDateString("ru-RU")} ${new Date(createdAt).toLocaleTimeString("ru-RU")}`,
    total: Number(
      raw?.payable_total ?? raw?.amount ?? raw?.grand_total ?? raw?.total ?? 0,
    ),
    items: items
      .map((item: any) => {
...
function syncSelectionWithMode() {
  ...
  const current = availableSales.value.find(
    (sale) => sale.id === selectedSaleId.value,
  );
  if (current) return;
...
function updateSelectedQuantity(
  item: SaleListItem["items"][number],
  event: Event,
) {
...
async function handlePrimaryAction() {
  ...
  if (isSaleActionMode.value) {
    ...
    toast.add({
      title:
        pickerMode.value === "exchange"
          ? "Режим обмена подготовлен"
          : "Режим возврата подготовлен",
      color: "success",
    });
    return;
  }
  ...
  try {
    if (pickerMode.value === "parked" || pickerMode.value === "draft") {
    ...
    toast.add({
      title:
        pickerMode.value === "parked" ? "Отложка открыта" : "Черновик открыт",
      color: "success",
    });
  } catch (error: any) {
    toast.add({
...
function extractSalesFromResponse(response: any) {
  ...
  if (Array.isArray(response?.data?.draft_sales))
    return response.data.draft_sales;
  if (Array.isArray(response?.data?.parked_sales))
    return response.data.parked_sales;
